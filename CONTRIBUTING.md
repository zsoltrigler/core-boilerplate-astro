...
- **Attribute forwarding** — every UI component forwards the global HTML attributes (`id`, `data-*`, `aria-*`, `hidden`, …) to its root element: `Props` extends the matching `HTMLAttributes<…>` and the rest of the props is spread onto the root via `...rest` (form fields forward to the underlying control instead). Attributes a component's own script or semantics depend on (e.g. `data-alert`, `role="alert"`) are spread _after_ `...rest` so they can't be overridden. Don't silently drop extra attributes in a new component.
